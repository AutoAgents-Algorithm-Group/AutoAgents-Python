...
def get_app_detail_api(
    ...
    url = f"{base_url}/api/agent/detail?agentId={agent_id}"
    response = requests.get(url, headers=headers)
    ...
    if response.status_code == 200:
        ...
        if response_data.get("code") == 1:
            ...
            raise Exception(f"获取智能体详情失败: {response_data.get('msg', 'Unknown error')}")
    else:
        raise Exception(f"获取智能体详情失败: {response.status_code} - {response.text}")